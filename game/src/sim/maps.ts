...
function nearSpots(
  ...
  switch (layout) {
    ...
    //
    // **점마다 따로 흔들지 않는다.** 이웃한 두 점의 간격이 `2·rv·sin(15°) ≈ 119` 로
    // `MIN_SPACING`(112)에 거의 붙어 있어서, 각도를 따로 흔들면 곧바로 하한을 깬다.
    // 대신 **고리 전체를 돌리고 반지름만 키운다** — 둘 다 점 사이 간격을 안 줄인다
    // (회전은 그대로, 반지름은 늘리기만 하므로 간격도 늘어난다).
    //
    // 2026-08-03까지 이 유형만 `rand` 를 한 번도 안 써서 **시드와 무관하게 늘 같은
    // 맵 하나**였다. 판의 약 1/5이 매번 똑같았다.
    case 'ring': {
      const cu = LONG / 2;
      const cv = SHORT / 2;
      // 반지름은 늘리기만 한다. 줄이면 간격이 `MIN_SPACING` 밑으로 떨어진다.
      const ru = 250 + rand() * 26;
      const rv = 230 + rand() * 22;
      // 고리 전체를 같은 각도만큼 돌린다. 상대 배치는 그대로라 간격 검사가 안 깨진다.
      const spin = (rand() * 2 - 1) * 9;
      // 내 쪽 절반만. 상대 쪽은 거울이 채운다.
      return [135, 165, 195, 225].map((deg) => {
        const a = ((deg + spin) * Math.PI) / 180;
