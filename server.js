...
/**
 * 명령을 몇 틱 뒤에 실행할 것인가. 30Hz 기준 15틱 = 0.5초.
 *
 * 배치 주기(전송 130ms)에 왕복 지연을 더한 값보다 커야 한다. 작으면 상대 입력이
 * 제때 안 와서 판이 멈추고(late batch), 크면 조작이 굼떠진다.
 *
 * **배속이 이 예산을 실시간으로 깎는다.** 15틱은 1배속 500ms 지만 1.5배속에선 333ms,
 * 2배속에선 250ms 다 (틱이 그만큼 빨리 지나므로). 12틱(1.5배속 267ms)에서 실기 정지가
 * 나서 15로 올렸다 (2026-08-06 사용자 신고). 그래도 멈추면 18(1.5배속 400ms)까지.
 * 대신 1배속 조작 지연이 함께 는다(0.4 → 0.5초).
 */
const INPUT_DELAY_TICKS = 15;
